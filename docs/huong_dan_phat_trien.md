...
graph TD
    ...
    Repository --> Service
    Service --> Controller
    Controller -->|ZodSerializerDto Lọc dữ liệu| ClientResponse([7. Client Response])
```
...
* **Lưu ý:** Sử dụng **`DTO`** để khai báo kiểu dữ liệu cho `@Body()` và sử dụng `@ZodSerializerDto` để lọc response.
...
export class WishlistController {
  ...
  @Post()
  @ZodSerializerDto(WishlistResDTO) // Lọc dữ liệu trả về theo schema
...
- [ ] **B7:** Viết file `.controller.ts` (Định nghĩa endpoint, `@Body() body: DTO`, `@ZodSerializerDto`).
